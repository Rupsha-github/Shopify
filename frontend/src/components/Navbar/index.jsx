...
import { useAuth } from "../../context/auth-context"; // Imported Auth context
    ...
    <header className="bg-orange-400 text-neutral-50 z-50 fixed top-0 left-0 w-full shadow-lg">
      ...
      <div className="flex items-center justify-between px-6 md:px-16 py-4 md:py-6 gap-4">
        ...
        <div onClick={() => navigate('/')} className="flex items-center gap-3 cursor-pointer shrink-0">
          ...
          <h1 className="text-2xl md:text-3xl font-bold hidden sm:block">Shopify</h1>
        </div>

        {/* Search Bar - Center Position (Only visible if logged in) */}
        {user && (
        ...
        <nav className="hidden md:flex gap-12 items-center shrink-0">
          {/* Home Icon */}
          <NavIcon 
            icon="home" 
            label="Home" 
